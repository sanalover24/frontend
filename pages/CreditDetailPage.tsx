...
import { PaymentMethod, CreditHistoryItem } from '../types';
import { useToast } from '../context/ToastContext';
import CustomSelect, { SelectOption } from '../components/ui/CustomSelect';
import { XIcon, LandmarkIcon, CreditCardIcon, ChevronLeftIcon, Trash2Icon } from '../components/icons';
import AmountInput from '../components/ui/AmountInput';

const AddReturnForm: React.FC<{ creditId: string, remainingAmount: number, onClose: () => void }> = ({ creditId, remainingAmount, onClose }) => {
    const { addCreditReturn, cards } = useData();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        addCreditReturn(creditId, returnAmount, paymentMethod, paymentMethod === 'card' ? cardId : undefined, note);
        addToast("Return payment added successfully!", 'success');
        onClose();
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="p-3 bg-slate-100 dark:bg-zinc-700/50 rounded-lg text-center">
                ...
                <p className="text-2xl font-bold text-rose-600 dark:text-rose-500">Rs. {remainingAmount.toLocaleString('en-IN')}</p>
            </div>
            <div>
                <label htmlFor="returnAmount" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Return Amount</label>
                <AmountInput id="returnAmount" value={amount} onChange={setAmount} className="mt-1 block w-full px-3 py-2 border rounded-lg bg-slate-50 dark:bg-zinc-700/50 border-slate-300 dark:border-zinc-600 text-zinc-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-500" required />
            </div>
             <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Received Via</label>
                <div className="mt-1 grid grid-cols-2 gap-2 rounded-md bg-slate-100 dark:bg-zinc-900 p-1">
                    <button type="button" onClick={() => setPaymentMethod('cash')} className={`flex items-center justify-center gap-2 px-4 py-2 rounded-md w-full transition-colors text-sm font-semibold ${paymentMethod === 'cash' ? 'bg-white dark:bg-zinc-700 shadow-sm text-zinc-800 dark:text-white' : 'text-zinc-600 dark:text-zinc-400'}`}>
                        <LandmarkIcon className="w-5 h-5" /> Cash
                    </button>
                    <button type="button" onClick={() => setPaymentMethod('card')} className={`flex items-center justify-center gap-2 px-4 py-2 rounded-md w-full transition-colors text-sm font-semibold ${paymentMethod === 'card' ? 'bg-white dark:bg-zinc-700 shadow-sm text-zinc-800 dark:text-white' : 'text-zinc-600 dark:text-zinc-400'}`}>
            ...
            {paymentMethod === 'card' && (
                <div className="animate-fade-in-up">
                    <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Select Card</label>
                    <CustomSelect value={cardId} onChange={setCardId} options={cardOptions} placeholder="Choose a card" />
                </div>
            )}
            <div>
                <label htmlFor="returnNote" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Note</label>
                <textarea id="returnNote" rows={2} value={note} onChange={e => setNote(e.target.value)} className="mt-1 block w-full px-3 py-2 border rounded-lg bg-slate-50 dark:bg-zinc-700/50 border-slate-300 dark:border-zinc-600 text-zinc-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="Optional details..." />
            </div>
...
const HistoryItem: React.FC<{ item: CreditHistoryItem }> = ({ item }) => {
    const { cards } = useData();
    ...
    return (
        <div className="flex items-start space-x-4">
             <div className={`mt-1 w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${isGiven ? 'bg-rose-100 dark:bg-rose-900/50' : 'bg-emerald-100 dark:bg-emerald-900/50'}`}>
                <span className="text-lg">{isGiven ? '💸' : '💰'}</span>
             </div>
             <div className="flex-grow">
                <p className={`font-bold ${isGiven ? 'text-rose-600 dark:text-rose-500' : 'text-emerald-600 dark:text-emerald-500'}`}>
                    {isGiven ? 'Given' : 'Returned'}: Rs. {item.amount.toLocaleString('en-IN')}
                </p>
                ...
                <p className="text-sm text-zinc-600 dark:text-zinc-300 capitalize">
                    Via {item.paymentMethod}{card ? ` (${card.cardName})` : ''}
                </p>
...
const CreditDetailPage: React.FC = () => {
  ...
  const { creditEntries, deleteCreditEntry } = useData();
  const { addToast } = useToast();
  const navigate = useNavigate();
  const [isReturnModalOpen, setIsReturnModalOpen] = useState(false);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);

  const entry = useMemo(() => creditEntries.find(e => e.id === id), [creditEntries, id]);
  ...
  const handleDelete = () => {
    ...
    addToast('Credit entry deleted successfully.', 'success');
    navigate('/credit');
  };
    ...
    <div className="space-y-6 animate-fade-in-up">
        ...
        <Card>
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                ...
                <div className={`text-xs font-bold px-3 py-1.5 rounded-full ${statusInfo[entry.status].color} text-white`}>
                    {isOverdue ? 'Overdue' : statusInfo[entry.status].text}
                </div>
            </div>
            <div className="mt-6">
                <div className="w-full bg-slate-200 dark:bg-zinc-700 rounded-full h-4">
                    <div className={`${statusInfo[entry.status].color} h-4 rounded-full transition-all duration-500`} style={{ width: `${progress}%` }}></div>
                </div>
                <div className="mt-2 grid grid-cols-3 text-sm font-medium">
                    ...
                    <div className={`text-right ${statusInfo[entry.status].textColor}`}>Remaining: Rs. {remaining.toLocaleString('en-IN')}</div>
                </div>
            </div>
            {entry.status !== 'completed' && (
                <div className="mt-6 text-center border-t dark:border-zinc-700 pt-4">
                    <Button onClick={() => setIsReturnModalOpen(true)}>Add Return Payment</Button>
                </div>
            )}
        </Card>

        <Card>
            <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-4">Credit History</h3>
            ...
            <div className="fixed inset-0 bg-black/75 z-50 flex justify-center items-center p-4 animate-fade-in" onClick={() => setIsReturnModalOpen(false)}>
                <Card className="w-full max-w-md animate-fade-in-up !p-0" style={{ animationDuration: '300ms' }} onClick={e => e.stopPropagation()}>
                    <div className="flex justify-between items-center p-4 sm:p-6 border-b dark:border-zinc-700">
                        <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">Add Return Payment</h2>
                        <button onClick={() => setIsReturnModalOpen(false)} className="text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200">
                            <XIcon className="w-6 h-6" />
                        </button>
                    </div>
                    <div className="p-4 sm:p-6">
                       <AddReturnForm creditId={entry.id} remainingAmount={remaining} onClose={() => setIsReturnModalOpen(false)} />
                    </div>
                </Card>
            </div>
        )}
                ...
                <Card className="w-full max-w-md animate-fade-in-up" style={{ animationDuration: '300ms' }} onClick={e => e.stopPropagation()}>
                    <div className="text-center">
                        ...
                        <div className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                            <p>Are you sure you want to delete this entire credit entry for <strong>{entry.personName}</strong>?</p>
                            <p className="mt-2">This action will remove the credit record but will not affect any transactions already added to your main balance.</p>
